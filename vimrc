" .vimrc
" Author: Jonathan Evans <jonathan.r.h.evans@gmail.com>
"
" Based on:
" slj dotfiles      https://github.com/sjl/dotfiles.git
...
set encoding=utf-8  " Override encoding incase not picked up from locale
set modelines=0  " No modelines at bottom of file specifying file settings
set autoindent  " Copy indentation from previous line for new lines
set showmode  " Indicate input or replace mode at bottom
set showcmd  " Show extra info about current command in bottom right
set hidden  " Allow switching to another buffer without saving current
set visualbell  " Use visual bell
set ttyfast  " Fast term conn. More chars redrawn instead of line based redraw
...
set lazyredraw  " Don't redraw when exec macros
set matchtime=3  " Time to highlight matching bracket (1/10s of a second)
set showbreak=↪  " Show linebreaks with an arrow
set splitbelow  " Split opens a buffer above by default. Set to below.
set splitright  " Split opens a buffer to the left by default. Set to right.
set fillchars=diff:⣿,vert:│  " Change the split boundary char
set autowrite  " Don't ask to save current buffer
set autoread  " Reload if an shell command is run from vim
set shiftround  " Round when shifting (reindenting with < >)
set title  " Make terminal inherit title from vim
...
" It's 2012. (Scroll visual lines rather than actual lines)
noremap j gj
noremap k gk
noremap gj j
noremap gk k
